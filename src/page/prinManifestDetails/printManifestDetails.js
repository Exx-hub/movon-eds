...
function TableView(props) {
  const busCompanyName =
    props && props.dataSource.length > 0
      ? props.dataSource[0].busCompanyName
      : "";
  const columns = [
    {
      title: () => (
        <span>
          (Movon) <br /> Bill of Lading
        </span>
      ),
    ...
    },
    {
      title: () => (
        <span>
          (
          {busCompanyName === "Bicol Isarog Trasport System Inc."
            ? "BITSI"
            : busCompanyName}
          ) <br /> Bill of Lading
        </span>
...
class PrintManifestDetails extends React.Component {
  ...
  componentDidMount() {
    ...
    ManifestService.getManifestByDate(
      ...
      const departureTime = moment(this.props.location.state.date).format(
        "MMM-DD-YYYY hh:mm A"
      );
      ...
      if (data && data.length > 0) {
        ...
        const dddd = data.map(
          (e) =>
            Number(e.priceDetails.totalPrice) -
            Number(e.priceDetails.convenienceFee)
        );
        totalSales = dddd.reduce(
          ...
          <div ref={this.printEl}>
            <div style={{ marginLeft: ".5rem", marginTop: ".5rem" }}>
              <div className="print-title-corner">
                ...
                <span className="print-company-date">
              ...
              <Row>
                ...
                <Col span={6}>
                  <h3 className="col-value">
                    {this.companyTag === "bicol-isarog"
                      ? `Edsa Cubao - ${routes2}`
                      : `${routes1}-${routes2}`}
                  </h3>
                ...
                <Col>
                  <h3 className="col-value">
                    ...
                    {this.state.totalSales
                      ? this.state.totalSales.toFixed(2)
                      : 0.0}
                  </h3>
