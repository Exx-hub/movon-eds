...
function DltbMatrix(props) {
  ...
  const parsePriceMatrix = (result) => {
    ...
    if (!errorCode) {
      ...
      if (!data) {
        ...
        fixMatrix =
          stringValue.fixMatrix.length > 0
            ? stringValue.fixMatrix
            : [{ ...props.data.FIX_PRICE_FORMAT }];
    ...
    requestUpdate(
      {
        ...
        stringValues: JSON.stringify({
          matrix,
          fixMatrix: item.fixMatrix || [],
        }),
      },
      () => {
  ...
  const updateFixPriceFiveStartMatrix = (values, data) => {
    ...
    const tempFixMatrixObject = {
      ...state.tempFixMatrixObject,
      fixMatrix: _fixMatrix,
    };
  ...
  const onSelect = async (name, val) => {
    ...
    switch (name) {
      ...
      case "origin-search":
        ...
        let tempOriginList = props.data.originList.filter((e) =>
          e.stationName.toLowerCase().includes(toSearch)
        );
        setState((e) => ({ ...e, tempOriginList, startName: toSearch }));
        break;

      case "destination-search":
        let tempDestination = state.originMatrix.filter((e) =>
          e.endStationName.toLowerCase().includes(val.toLowerCase())
        );
        busPartner.parseMatrixDataSource(tempDestination);
        setState((e) => ({
          ...e,
          tempDestination,
          destination: val.toLowerCase(),
        }));
        break;

      case "search-fixMatrixOriginStation":
        let fixPriceOriginList = props.data.originList.filter((e) =>
          e.stationName.toLowerCase().includes(val.toLowerCase())
        );
        setState((e) => ({
          ...e,
          fixPriceOriginList,
          fixMatrixOriginName: val.toLowerCase(),
        }));
        break;

      case "search-fixMatrixDestinationStation":
        let tempDestinationList = state.destinationList.filter((e) =>
          e.stationName.toLowerCase().includes(val.toLowerCase())
        );
        setState((e) => ({
          ...e,
          tempDestinationList,
          fixMatrixDestinationName: val.toLowerCase(),
        }));
        break;
  ...
  const MatrixModalWeightRange = () => {
    ...
    switch (busPartner.getName()) {
      case "isarog-liner":
        view = (
          <BicolIsarogMatrixModalWeightRange
            {...props}
            cancelText="Ok"
            data={weightRangeModal.data}
            onCancel={() =>
              setWeightRangeModal((e) => ({
                ...e,
                visible: false,
                data: undefined,
              }))
  ...
  const FixMatrixModalContainer = (props) => {
    ...
    switch (UserProfile.getBusCompanyTag()) {
      case "dltb":
        View = (
          <AddFixMatrixModalContent
            ...
            okText={
              fixPriceModal.type === "delete" ? fixPriceModal.type : "Save"
            }
            cancelText="Cancel"
      ...
      case "isarog-liner":
        View = (
          <IsarogLinerFixMatrixModalContent
            {...props}
            type={fixPriceModal.type}
            okText={
              fixPriceModal.type === "delete" ? fixPriceModal.type : "Save"
    ...
    <div>
      ...
      <div style={{ padding: "1rem" }}>
        <Collapse defaultActiveKey={["1"]} style={{ marginBottom: "2.5rem" }}>
          <Panel
            style={{ background: getHeaderContainer() }}
            header="Price Matrix"
            key="1"
          >
            <div
              ...
              <Space direction="vertical">
                ...
                >
                  ...
                  {state.tempOriginList
                    ...
                    .map((e, i) => (
                      <Option key={i} value={e.stationId}>
                        {e.stationName}
                      </Option>
                    ))}
                </AutoComplete>
        ...
        <Collapse>
          <Panel
            style={{ background: getHeaderContainer(), fontColor: "#fff" }}
            header="Fix Price Matrix"
            key="1"
          >
            <div
              style={{
            ...
            >
              ...
              <Space direction="vertical">
                ...
                <AutoComplete
                  ...
                  style={{ width: 300, marginBottom: "1rem" }}
                  onSearch={(e) =>
                    onSelect("search-fixMatrixDestinationStation", e)
                  }
                  onSelect={(e) => onSelect("fixMatrixDestinationName", e)}
                >
                  {state.tempDestinationList.map((e, i) => (
                    <Option key={i} value={e.stationId}>
                      {e.stationName}
                    </Option>
                  ))}
            ...
            {
              <>
                {busPartner.getFixPriceTableComponent((c) =>
                  broadcastListener(c)
                )}
              </>
            }
