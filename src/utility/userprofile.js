...
class UserProfileClass {
  ...
  // this is questionable. i think this is not supposed to be a function invocaation. will look into
  ...
  getBusCompanyTag() {
    if (this.getBusCompany()) {
      let tag =
        this.getBusCompany().tag ||
        (this.getBusCompany().config.parcel &&
          this.getBusCompany().config.parcel.tag) ||
        undefined;
  ...
  getDiscount() {
    if (this.getBusCompany()) {
      return (
        (this.getBusCompany().config && this.getBusCompany().config.discount) ||
        []
      );
    }
    return [];
  ...
  getStickerCount() {
    if (this.getBusCompany()) {
      const count =
        (this.getBusCompany().config &&
          this.getBusCompany().config.noOfStickerCopy) ||
        1;
      return count;
    }
    return 1;
